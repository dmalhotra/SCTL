...
template <template <class...> class DevVec, class Policy>
void ensureInverse(const Policy& pol, Plan<DevVec>& s) {
  if (s.inv) return;
...
void ensureRecut(Plan<DevVec>& s, const Comm& comm) {
...
void forward(const Policy& pol, const T* src, T* dst, Plan<DevVec>& s, Long dof, const Comm& comm) {
  if (comm.Size() == 1) {  // stages 2-4 are absent, so the sort alone is the map
    localMove<DevVec>(pol, src, dst, s.pre, s.Nloc, dof);
    return;
  }
  ensureRecut(s, comm);
  ...
  a.resize(s.Nloc * dof);
  b.resize(s.Nmid * dof);
  localMove<DevVec>(pol, src, thrust::raw_pointer_cast(a.data()), s.pre, s.Nloc, dof);
  exchange<DevVec>(pol, thrust::raw_pointer_cast(a.data()), s.Nloc,
                         thrust::raw_pointer_cast(b.data()), s.Nmid, s.scnt, s.rcnt, dof, comm);
  if (!s.recut) {
    localMove<DevVec>(pol, thrust::raw_pointer_cast(b.data()), dst, s.post, s.Nmid, dof);
    return;
  }
  a.resize(s.Nmid * dof);
  localMove<DevVec>(pol, thrust::raw_pointer_cast(b.data()), thrust::raw_pointer_cast(a.data()), s.post, s.Nmid, dof);
  exchange<DevVec>(pol, thrust::raw_pointer_cast(a.data()), s.Nmid, dst, s.Ntree, s.rscnt, s.rrcnt, dof, comm);
}

/** Sorted order -> caller order: the same stages in reverse, each local one using its inverse. */
template <class T, template <class...> class DevVec, class Policy>
void reverse(const Policy& pol, const T* src, T* dst, Plan<DevVec>& s, Long dof, const Comm& comm) {
  ensureInverse(pol, s);
  if (comm.Size() == 1) {
    localMove<DevVec>(pol, src, dst, s.pre_inv, s.Nloc, dof);
    return;
  }
  ensureRecut(s, comm);
  DevVec<T>& a = detail::PersistentBuffer<T, DevVec, detail::Buf::PtSend>();
  ...
  if (s.recut) {
    b.resize(s.Nmid * dof);
    exchange<DevVec>(pol, src, s.Ntree, thrust::raw_pointer_cast(b.data()), s.Nmid, s.rrcnt, s.rscnt, dof, comm);
    mid = thrust::raw_pointer_cast(b.data());
  }
  a.resize(s.Nmid * dof);
  localMove<DevVec>(pol, mid, thrust::raw_pointer_cast(a.data()), s.post_inv, s.Nmid, dof);
  b.resize(s.Nloc * dof);
  exchange<DevVec>(pol, thrust::raw_pointer_cast(a.data()), s.Nmid,
                         thrust::raw_pointer_cast(b.data()), s.Nloc, s.rcnt, s.scnt, dof, comm);
  localMove<DevVec>(pol, thrust::raw_pointer_cast(b.data()), dst, s.pre_inv, s.Nloc, dof);
}
...
void SortScatter<Key, DevVec>::ScatterForward(const T* src, T* dst, Long dof) const {
  detail_sortScatter::forward(detail::scratch_policy<DevVec, T>(), src, dst, plan_, dof, comm_);
}

template <class Key, template <class...> class DevVec> template <class T>
void SortScatter<Key, DevVec>::ScatterReverse(const T* src, T* dst, Long dof) const {
  detail_sortScatter::reverse(detail::scratch_policy<DevVec, T>(), src, dst, plan_, dof, comm_);
}
