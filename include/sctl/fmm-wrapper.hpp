...
template <class Real, Integer DIM> class ParticleFMM {
  ...
  public:
    ...
    static void test(const Comm& comm) {
      ...
      Vector<Real> trg_coord(50000*DIM);
      Vector<Real>  sl_coord(50000*DIM);
      Vector<Real>  dl_coord(50000*DIM);
      Vector<Real>  dl_norml(50000*DIM);
  ...
  private:
    ...
    #ifdef SCTL_HAVE_PVFMM
    template <class SCTLKernel, bool use_dummy_normal=false> struct PVFMMKernelFn : public pvfmm::GenericKernel<PVFMMKernelFn<SCTLKernel, use_dummy_normal>> {
    ...
    void EvalPVFMM(Vector<Real>& U, const std::string& trg_name) const {
      ...
      static_assert(!std::is_same<Real,Real>::value, "Require MPI to use PVFMM");
      #endif
      #if !defined(SCTL_HAVE_BLAS)
      static_assert(!std::is_same<Real,Real>::value, "Require BLAS to use PVFMM");
      #endif
      #if !defined(SCTL_HAVE_LAPACK)
      static_assert(!std::is_same<Real,Real>::value, "Require LAPACK to use PVFMM");
      #endif
      #if !defined(SCTL_HAVE_FFTW)
      static_assert(!std::is_same<Real,double>::value, "Require FFTW to use PVFMM");
      #endif
      #if !defined(SCTL_HAVE_FFTWF)
      static_assert(!std::is_same<Real,float>::value, "Require FFTWF to use PVFMM");
      #endif
      #if !defined(SCTL_HAVE_FFTWL)
      static_assert(!std::is_same<Real,long double>::value, "Require FFTWL to use PVFMM");
      #endif
      ...
      const Long Nt = Xt.Dim() / DIM;
      SCTL_ASSERT(Xt.Dim() == Nt * DIM);
      if (Nt < 40000) return EvalDirect(U, trg_name);
