...
\subsection{Intel Icelake}

Not yet run. \texttt{scripts/bench-cubed-sphere-icelake.sbatch} submits the same no-argument run
to an Icelake node with 64 cores:
\begin{center}\small\texttt{sbatch scripts/bench-cubed-sphere-icelake.sbatch}\end{center}
The script builds into \texttt{bin.icelake/} and \texttt{obj.icelake/} rather than \texttt{./bin}.
That is not tidiness: the Makefile compiles with \texttt{-march=native}, so an Icelake binary
carries AVX-512 and dies with \texttt{SIGILL} on the Zen 2 workstations, and the two builds share
a home filesystem.
