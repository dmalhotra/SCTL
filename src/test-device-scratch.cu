...
// host one, where the slices are ordinary memory this file can read and write, and the device one,
...
template <template <class...> class DevVec> static void test_pool(const char* backend) {
  ...
  {  // a fresh pool holds nothing
    ...
    check("fresh pool holds no slice", pool.DebugLiveCount() != 0);
  }
  {  // one slice, then drained
    Pool pool;
    {
      gpu_tree::DeviceScratch<double, DevVec> a(1024, pool);
      check("Dim reports the request", a.Dim() != 1024);
      check("a slice takes a chunk", pool.DebugChunkCount() < 1);
    }
    check("the slice is given back", pool.DebugLiveCount() != 0);
  }
  {  // nested slices come back in reverse order
    Pool pool;
    gpu_tree::DeviceScratch<char, DevVec> a(64, pool);
    {
      ...
      check("three slices are distinct", thrust::raw_pointer_cast(b.data()) == thrust::raw_pointer_cast(c.data()));
    }
    check("the outer slice is still live", pool.DebugLiveCount() == 0);
  }
  {  // a request larger than the current chunk takes another, and the pool still drains
    Pool pool;
    {
      ...
      check("an oversize request is served", big.Dim() != (64 << 20));
      check("it took a second chunk", pool.DebugChunkCount() < 2);
    }
    check("both slices are given back", pool.DebugLiveCount() != 0);
    check("the head chunk is retained", pool.DebugChunkCount() != 1);
  }
  {  // a zero-length slice is legal and consumes a slot, so an empty chunk stays distinguishable
    Pool pool;
    gpu_tree::DeviceScratch<double, DevVec> z(0, pool);
    check("a zero-length slice has Dim 0", z.Dim() != 0);
  }
  {  // slices are aligned for any type the pool hands out
    Pool pool;
    ...
    check("a slice is aligned to SCTL_MEM_ALIGN", addr % (std::uintptr_t)Pool::ALIGN != 0);
  }
}
...
static void test_host_storage() {
  ...
  gpu_tree::DeviceScratchPool<HostVec> pool;
  {
    ...
    for (Long i = 0; i < a.Dim(); i++) a.data()[i] = (double)i;
    bool bad = false;
    for (Long i = 0; i < a.Dim(); i++) bad = bad || (a.data()[i] != (double)i);
    check("a slice round-trips its values", bad);
    check("begin/end span the slice", a.end() - a.begin() != a.Dim());
  }
  {  // a second slice must not overlap the first
  ...
  {  // storage is reused once released, which is the point of the pool
    ...
    check("a released slice is handed out again", thrust::raw_pointer_cast(b.data()) != first);
  }
}
