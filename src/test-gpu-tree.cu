...
template <class Real, Integer DIM, template <class...> class DevVec> Long test_vs_sctl() {
  ...
  { // node set, and at one rank flags, lists and partition, for every periodicity mask with and without 2:1 balance
    ...
    for (sctl::PeriodicityT m = 0; m < (1 << DIM); m++) for (Integer b21 = 0; b21 <= 1; b21++) {
      ...
      sctl::Tree<DIM> st(comm); st.UpdateRefinement(x, 32, b21, per, halo);
      Long gb, ge, sb, se;
  ...
  { // particles: two groups, data filled through the view, data added after a repartition, re-cut of a re-cut
    ...
    gt.UpdateRefinement(xd, 32, true, sctl::Periodicity::NONE, 0); st.UpdateRefinement(x, 32, true, sctl::Periodicity::NONE, 0);
    gt.AddParticles("pt", xd); gt.AddParticleData("f", "pt", fd);
    st.AddParticles("pt", x); st.AddParticleData("f", "pt", f);
    gt.AddParticles("q", y2d); gt.AddParticleData("g", "q", gd);
    st.AddParticles("q", y2s); st.AddParticleData("g", "q", g);
    ...
    gt.UpdateRefinement(yd, 25, true, sctl::Periodicity::NONE, 0); st.UpdateRefinement(y, 25, true, sctl::Periodicity::NONE, 0);
    gt.AddParticleData("h", "pt", hd); st.AddParticleData("h", "pt", h);  // added after the repartition: the forward scatter with its re-cut stage
